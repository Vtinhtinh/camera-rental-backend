...
const { sendPaymentConfirmationNotification, sendPendingPaymentAlert } = require('../config/telegram');
...
const paymentController = {
  ...
  async createVietQRPayment(req, res) {
    try {
      ...
      await sendPendingPaymentAlert(payment, bookingInfo);
  ...
  async adminConfirmPayment(req, res) {
    try {
      ...
      if (payment.isExpired() && payment.status === VietQRPayment.PAYMENT_STATUS.PENDING) {
