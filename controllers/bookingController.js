...
const calculatePricing = (startDate, endDate, product, rentalType = 'day', hours = null) => {
  ...
  if (rentalType === 'hour' && hours) {
    ...
    if (hours === 3) {
      pricingType = '3h';
      unitPrice = product.pricing.price3h;
      totalPrice = unitPrice;
    } else if (hours === 6) {
      pricingType = '6h';
      unitPrice = product.pricing.price6h;
      totalPrice = unitPrice;
    } else if (hours === 12) {
      pricingType = '12h';
      unitPrice = product.pricing.price12h;
      totalPrice = unitPrice;
    } else {
      // Default fallback
      pricingType = '6h';
  ...
  // Day rental mode (original logic)
  ...
  if (hoursDiff <= 6) {
    ...
    if (days === 1) {
      pricingType = '1d';
      pricePerDay = product.pricing.price1d;
    } else if (days === 2) {
      pricingType = '2d';
      pricePerDay = product.pricing.price2d;
    } else {
      pricingType = '3d+';
      pricePerDay = product.pricing.price3dPlus;
    }
  ...
  return {
    pricing: { type: pricingType, unitPrice },
    totalPrice,
    rentalDays: Math.max(1, Math.ceil(hoursDiff / 24))
  };
};

const createBooking = async (req, res, next) => {
  try {
    const { productId, startDate, endDate, customerName, customerPhone, customerEmail, notes, deliveryAddress, rentalType, hours, identityDocuments } = req.body;
